...
function CollectionsPage() {
  ...
  const seeFiltersFunc = () => {
    if (device === "mobile") {
      return seeFilters ? "max-h-[500px] overflow-hidden" : "max-h-0 overflow-hidden";
    }
      ...
      >
        <div
          className={`flex relative flex-col ${
            device === "mobile" ? "w-full bg-black z-50 pt-[12px]" : "w-[241px] py-[12px]"
        ...
        <div className="grid flex-1 gap-[10px] grid-cols-1 md:grid-cols-2 lg:grid-cols-3	h-min justify-center overflow-hidden p-0 w-full">
          {collectionList?.map((collection, i) => (
            <Link key={i} href={`/collections/${collection.id}`}>
              <Card
                key={collection.id}
                className="bg-[rgb(10,10,10)] rounded-lg text-white overflow-hidden border-0 hover:brightness-125 duration-300 cursor-pointer"
              >
